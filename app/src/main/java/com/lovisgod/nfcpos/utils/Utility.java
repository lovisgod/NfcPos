...
public class Utility {
    ...
    public static String prettyPrintHex(byte[] data, int indent) {
        return Utility.prettyPrintHex(Utility.byteArrayToHexString(data), indent, true);
    }
    
    public static String prettyPrintHex(byte[] data) {
        return Utility.prettyPrintHex(Utility.byteArrayToHexString(data), 0, true);
    }
    
    public static String prettyPrintHex(byte[] data, int startPos, int length) {
        return Utility.prettyPrintHex(Utility.byteArrayToHexString(data, startPos, length), 0, true);
    }

    public static String prettyPrintHexNoWrap(byte[] data) {
        return Utility.prettyPrintHex(Utility.byteArrayToHexString(data), 0, false);
    }
    
    public static String prettyPrintHexNoWrap(byte[] data, int startPos, int length) {
        return Utility.prettyPrintHex(Utility.byteArrayToHexString(data, startPos, length), 0, false);
    }
    
    public static String prettyPrintHexNoWrap(String in) {
        return Utility.prettyPrintHex(in, 0, false);
    }
    ...
    public static int byteArrayToInt(byte[] byteArray, int startPos, int length) {
        ...
        if (length == 4 && Utility.isBitSet(byteArray[startPos], 8)){
            throw new IllegalArgumentException("Signed bit is set (leftmost bit): " + Utility.byte2Hex(byteArray[startPos]));
        }
        int value = 0;
        for (int i = 0; i < length; i++) {
    ...
    public static long byteArrayToLong(byte[] byteArray, int startPos, int length) {
        ...
        if (length == 8 && Utility.isBitSet(byteArray[startPos], 8)){
            throw new IllegalArgumentException("Signed bit is set (leftmost bit): " + Utility.byte2Hex(byteArray[startPos]));
        }
        long value = 0;
        for (int i = 0; i < length; i++) {
    ...
    public static byte[] intToBinaryEncodedDecimalByteArray(int val){
        ...
        return Utility.fromHexString(str);
    }
    ...
    public static int binaryHexCodedDecimalToInt(byte[] bcdArray) {
        ...
        return binaryHexCodedDecimalToInt(Utility.byteArrayToHexString(bcdArray));
    }
    ...
    public static String byte2BinaryLiteral(byte val) {
        String s = Integer.toBinaryString(Utility.byteToInt(val));
        if (s.length() < 8) {
    ...
    public static Class getCallerClass(int i) {
        ...
        if (classContext != null) {
            for (int j = 0; j < classContext.length; j++) {
                if (classContext[j] == Utility.class) {
            ...
            try {
                ...
                for (int j = 0; j < classNames.length; j++) {
                    if (Class.forName(classNames[j].getClassName()) == Utility.class) {
...
//        System.out.println(Util.byteArrayToLong(Util.fromHexString("22 18 09 04 0b 00 e0 30 23 07 00 00 00 42 d2 85 4e 23 07 00 00 00 00 21 69 42"), 13, 4));
        ...
        System.out.println(Utility.prettyPrintHexNoWrap(Utility.resizeArray(new byte[]{0x01}, 0)));
        System.out.println(Utility.prettyPrintHexNoWrap(Utility.resizeArray(new byte[]{0x01}, 1)));
        System.out.println(Utility.prettyPrintHexNoWrap(Utility.resizeArray(new byte[]{0x01}, 2)));
        
        System.out.println(Utility.prettyPrintHexNoWrap(Utility.resizeArray(new byte[]{0x01, 0x02}, 1)));
        System.out.println(Utility.prettyPrintHexNoWrap(Utility.resizeArray(new byte[]{0x01, 0x02}, 4)));
    }
}